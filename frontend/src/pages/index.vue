...
onMounted(async () => {
  ...
  for (const record of records.value) {
    const geoLocation = await fetchGeoLocation(record.RemoteAddr);
    const marker = L.marker([geoLocation.lat, geoLocation.lon]).addTo(map);
    marker.bindPopup(`风险等级: ${record.Risk}`).openPopup();
  }
});
</script>
...
.dashboard {
  ...
  color: white;
  width: 100%;
  box-sizing: border-box; /* 添加这行以确保padding不会导致超出宽度 */
}
...
.last7days-chart-container {
  ...
  border-radius: 8px;
  width: 100%;
  max-width: 100%; /* 确保宽度不会超过容器 */
  border: 2px solid #3a3f5c; /* 增加边框 */
...
.chart {
  height: 80vh; /* 使用视口高度，确保图表根据屏幕大小自适应 */
  width: 100%;
  max-width: 100%;
}

.last7days-chart {
  height: 40vh; /* 使用视口高度，确保图表根据屏幕大小自适应 */
  width: 100%;
  max-width: 100%;
}

.map-container {
  height: 80vh; /* 使用视口高度，确保地图根据屏幕大小自适应 */
  width: 100%;
  max-width: 100%;
  margin-top: 20px;
  border: 2px solid #3a3f5c; /* 增加边框 */
